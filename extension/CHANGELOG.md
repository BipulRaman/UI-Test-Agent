...
### Fixed
- **No more spurious "npx not found" warning.** The probe still records npx detection in the diagnostic log, but it no longer raises a user-facing notification. npx ships with Node \u2265 7, so if Node is detected we trust the launch path \u2014 VS Code's MCP spawn uses a shell and is more lenient than our probe (different shim filenames across Volta / fnm / nvm-windows, PATHEXT, stale PATH in already-running VS Code processes). Only **missing Node.js** is now treated as fatal.
- **More robust npx detection (still kept for diagnostics).** Tries every plausible Windows shim filename (`npx.cmd`, `npx.exe`, `npx.ps1`, plain `npx`), falls back to a shell-based lookup so `PATHEXT` is honored, and as a last resort scans the directory next to the resolved `node` binary.
